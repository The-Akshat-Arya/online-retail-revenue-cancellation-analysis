...
SELECT
    (SELECT COUNT(DISTINCT invoice_no)
     FROM sales_transactions) AS completed_orders,

    (SELECT COUNT(DISTINCT invoice_no)
     FROM cancelled_transactions) AS cancelled_orders,

    (SELECT COUNT(DISTINCT customer_id)
     FROM sales_transactions
     WHERE customer_id <> -1) AS known_customers,

    (SELECT COUNT(DISTINCT stock_code)
     FROM sales_transactions) AS products_sold,

    (SELECT ROUND(SUM(revenue), 2)
     FROM sales_transactions) AS gross_sales,

    (SELECT ROUND(SUM(cancellation_value), 2)
     FROM cancelled_transactions) AS cancellation_value,

    ROUND(
        (SELECT SUM(revenue) FROM sales_transactions)
        -
        (SELECT SUM(cancellation_value)
    ...
    ROUND(
        ...
        2
    ) AS order_cancellation_rate_pct,
...
-- =====================================================
-- 2. MONTHLY SALES AND CANCELLATION PERFORMANCE
-- =====================================================
...
SELECT
    ...
    COALESCE(cancel.cancelled_orders, 0)
        AS cancelled_orders,

    COALESCE(cancel.cancellation_value, 0)
    ...
    CASE
        WHEN sales.invoice_year_month = '2011-12'
            THEN 'Partial month through December 9'
        ELSE 'Full month'
...
-- December 2011 is excluded because it is incomplete.
-- =====================================================
...
-- =====================================================
-- 5. TOP MERCHANDISE PRODUCTS
-- Removes obvious service and fee-style stock codes.
-- =====================================================
...
-- =====================================================
-- 6. PRODUCTS WITH HIGHEST CANCELLATION VALUE
-- =====================================================
...
-- Unknown customers (-1) are excluded.
-- =====================================================
...
-- =====================================================
-- 8. KNOWN VS ANONYMOUS CUSTOMER REVENUE
-- =====================================================

SELECT
    CASE
        WHEN customer_id = -1 THEN 'Anonymous Customer'
        ELSE 'Known Customer'
